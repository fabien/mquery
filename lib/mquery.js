...
function Query(criteria, options) {
  ...
  this.$useProjection = true; // enforced
...
Query.prototype._findAndModify = function(type, callback) {
  ...
  if ('remove' != type) {
    if (!('new' in options)) options.returnOriginal = false;
  ...
  if ('remove' == type) {
    this._collection.findOneAndDelete(conds, options, utils.tick(callback));
  } else {
    this._collection.findOneAndUpdate(conds, doc, options, utils.tick(callback));
  }
